...
class MacdAlphaModel(AlphaModel):
    ...
    def __init__(self,
        ...
        self.insightPeriod =timedelta(minutes =1) #Time.Multiply(Extensions.ToTimeSpan(resolution), fastPeriod)
    ...
    def Update(self, algorithm, data):
        ...
        for symbol, symbolData in self.symbolDataBySymbol.items():
          #  direction = InsightDirection.Flat
            #algorithm.Consolidate(symbol, timedelta(minutes=45), self.FortyFiveMinuteBarHandler)
          ...
          #  algorithm.Debug(UP_BB_4hr)
           ...
           # bb2 = MA - 2*ST
            ...
            ###############
            if not (self.macdWindows[symbol].IsReady and self.closeWindows[symbol].IsReady and symbolData.MACD):return
           ...
           # previous_state = symbolData.State
            ...
            if symbolData.bool_4hr.IsReady and macd.IsReady:
                if ((DW_BB_4hr/self.fxClosed)*100>99 and (DW_BB_4hr/self.fxClosed)*100<=100):
                    algorithm.Debug(i+1)
                    signal=self.Macd(symbol)
                   # if signal=="BUY":
                    if macd.Current.Value>0:
                        if symbolData.ADX.Current.Value>25:
                            signad=self.Macd_min(symbol)
                            if signad=="BUY":
                                if fast<slow:
                                    #direction=InsightDirection.Flat
                                    insights.append(Insight.Price(symbol,self.insightPeriod, InsightDirection.Flat))
                                elif ((DW_BB_m/self.fxClosed)*100>99 and (DW_BB_m/self.fxClosed)*100<=100):
               ...
               # if (self.fxClose <self.lwma_4hr.Current.Value) or (self.fxClose <self.lwma_hr.Current.Value):
                elif ((self.fxClosed/UP_BB_4hr)*100>99 and (self.fxClosed/UP_BB_4hr)*100<=100):
                    algorithm.Debug(i+1)
                    signal=self.Macd(symbol)
                    if signal=="SELL":
                    if macd.Current.Value<0:
                        if symbolData.ADX.Current.Value>25:
                            ...
                            if signal=="SELL":
                                if fast>slow:
                                  #  direction=InsightDirection.Flat
                                     insights.append(Insight.Price(symbol,self.insightPeriod, InsightDirection.Flat))
                                elif ((self.fxClosed/UP_BB_m)*100>99 and (self.fxClosed/UP_BB_m)*100<=100):
